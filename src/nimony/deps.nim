...
proc targetFile(config: NifConfig; f: FilePair, ext: string): string = config.nifcachePath / f.modname & "." & ext
...
proc generateFinalBuildFile(c: DepContext; commandLineArgsNifc: string; passC, passL: string, target: string): string =
...
proc buildGraph*(config: sink NifConfig; project: string; forceRebuild, silentMake: bool;
    ...
    passC, passL: string, executableArgs: string, target: string) =
